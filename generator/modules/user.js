...
async function add_user_activity() {
    ...
    try {
        ...
        for (let i = 0; i < users.length; i += 1) {
            let movies = (await connection.execute('SELECT id FROM movies ORDER BY RAND() LIMIT ?', [Math.round(Math.random() * 10)]))[0];
            ...
            let series = (await connection.execute('SELECT id FROM series ORDER BY RAND() LIMIT ?', [Math.round(Math.random() * 10)]))[0];
            ...
            let games = (await connection.execute('SELECT id FROM games ORDER BY RAND() LIMIT ?', [Math.round(Math.random() * 10)]))[0];
